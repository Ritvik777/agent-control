"""Tests for JSON validation plugin."""

import pytest
from agent_control_models import JSONControlEvaluatorPluginConfig
from agent_control_plugins.builtin.json import JSONControlEvaluatorPlugin
...
class TestJSONParsing:
    ...
    async def test_dict_input(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(required_fields=["id"]))
        result = await plugin.evaluate({"id": 123})
    ...
    async def test_invalid_json_blocked_by_default(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(required_fields=["id"]))
        result = await plugin.evaluate("{invalid json")
    ...
    async def test_invalid_json_allowed_when_configured(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(required_fields=["id"], allow_invalid_json=True)
        )
        result = await plugin.evaluate("{invalid json")
...
class TestSchemaValidation:
    ...
    async def test_valid_schema(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(json_schema=schema))
        result = await plugin.evaluate({"id": 1, "name": "test"})
        assert result.matched is False  # Validation passed
    ...
    async def test_invalid_schema_missing_required(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(json_schema=schema))
        result = await plugin.evaluate({"id": 1})
    ...
    async def test_invalid_schema_wrong_type(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(json_schema=schema))
        result = await plugin.evaluate({"id": "not-an-int"})
        assert result.matched is True  # Failed
        assert "Schema validation failed" in result.message
    ...
    async def test_nested_object_validation(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(json_schema=schema))
        result = await plugin.evaluate({"user": {"id": 123}})
        assert result.matched is False  # Validation passed
    ...
    async def test_array_validation(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(json_schema=schema))
        result = await plugin.evaluate([1, 2, 3])
        assert result.matched is False  # Validation passed

        result = await plugin.evaluate([1, "not-int", 3])
        assert result.matched is True  # Failed
...
class TestRequiredFieldsValidation:
    ...
    async def test_all_present(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(required_fields=["id", "name", "email"]))
        result = await plugin.evaluate({"id": 1, "name": "test", "email": "test@example.com"})
        assert result.matched is False  # Validation passed

    @pytest.mark.asyncio
    async def test_missing_field(self):
        """Test that missing required field fails validation."""
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(required_fields=["id", "name"]))
        result = await plugin.evaluate({"id": 1})
        assert result.matched is True  # Failed
    ...
    async def test_null_allowed(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(required_fields=["id"], allow_null_required=True)
        )
        result = await plugin.evaluate({"id": None})
        assert result.matched is False  # Validation passed
    ...
    async def test_null_disallowed(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(required_fields=["id"], allow_null_required=False)
        )
        result = await plugin.evaluate({"id": None})
        assert result.matched is True  # Failed
    ...
    async def test_nested_required_fields(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(required_fields=["user.id", "user.email"]))
        result = await plugin.evaluate({"user": {"id": 123, "email": "test@example.com"}})
        assert result.matched is False  # Validation passed
...
class TestTypesValidation:
    ...
    async def test_type_mismatch(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(field_types={"id": "string"}))
        result = await plugin.evaluate({"id": 123})
        assert result.matched is True  # Failed
    ...
    async def test_missing_field(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(field_types={"id": "string"}))
        result = await plugin.evaluate({"name": "test"})
        assert result.matched is True  # Failed
    ...
    async def test_nested_field_types(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_types={"user.id": "integer", "user.name": "string"})
        )
        result = await plugin.evaluate({"user": {"id": 123, "name": "test"}})
        assert result.matched is False  # Validation passed
    ...
    async def test_extra_fields_allowed(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_types={"id": "string"}, allow_extra_fields=True)
        )
        result = await plugin.evaluate({"id": "123", "extra": "field"})
        assert result.matched is False  # Validation passed
    ...
    async def test_extra_fields_denied(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_types={"id": "string"}, allow_extra_fields=False)
        )
        result = await plugin.evaluate({"id": "123", "extra": "field"})
        assert result.matched is True  # Failed
    ...
    async def test_nested_fields_with_strict_mode_no_extra_fields(self):
        ...
        This was a critical bug where specifying field_types={"user.id": "string"} with
        allow_extra_fields=False would incorrectly flag "user" as an extra field, even though
        it's just a container for the typed field "user.id".
        """
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(
    ...
    async def test_multiple_nested_levels_strict_mode(self):
        ...
        result = await plugin.evaluate({"user": {"profile": {"email": "test@example.com"}}})
        assert result.matched is False  # Validation passed
    ...
    async def test_nested_fields_with_required_and_strict_mode(self):
        ...
        When both required_fields and field_types are specified with allow_extra_fields=False,
        the plugin should allow fields that are either in field_types OR required_fields.
        """
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(
...
class TestConstraintsValidation:
    ...
    async def test_numeric_range_within_bounds(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_constraints={"score": {"min": 0.0, "max": 1.0}})
        )
        result = await plugin.evaluate({"score": 0.75})
        assert result.matched is False  # Validation passed
    ...
    async def test_numeric_range_below_min(self):
        ...
        result = await plugin.evaluate({"score": -0.5})
        assert result.matched is True  # Failed
    ...
    async def test_numeric_range_above_max(self):
        ...
        result = await plugin.evaluate({"score": 1.5})
        assert result.matched is True  # Failed
    ...
    async def test_integer_range(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_constraints={"count": {"min": -10, "max": 5}})
        )
        result = await plugin.evaluate({"count": 3})
        assert result.matched is False  # Validation passed

        result = await plugin.evaluate({"count": 10})
        assert result.matched is True  # Failed
    ...
    async def test_enum_valid_value(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_constraints={"status": {"enum": ["pending", "approved", "rejected"]}})
        )
        result = await plugin.evaluate({"status": "approved"})
        assert result.matched is False  # Validation passed
    ...
    async def test_enum_invalid_value(self):
        ...
        result = await plugin.evaluate({"status": "invalid"})
        assert result.matched is True  # Failed
    ...
    async def test_string_length_within_range(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_constraints={"username": {"min_length": 3, "max_length": 20}})
        )
        result = await plugin.evaluate({"username": "test_user"})
        assert result.matched is False  # Validation passed
    ...
    async def test_string_length_too_short(self):
        ...
        result = await plugin.evaluate({"username": "ab"})
        assert result.matched is True  # Failed
        assert "below minimum" in result.message
    ...
    async def test_string_length_too_long(self):
        ...
        result = await plugin.evaluate({"username": "a" * 25})
        assert result.matched is True  # Failed
        assert "above maximum" in result.message
    ...
    async def test_mixed_constraints(self):
        ...
        result = await plugin.evaluate({"score": 0.8, "status": "active", "name": "Test"})
        assert result.matched is False  # Validation passed
...
class TestPatternMatching:
    ...
    async def test_all_patterns_match(self):
        ...
        result = await plugin.evaluate({"email": "test@example.com", "phone": "+1234567890"})
        assert result.matched is False  # Validation passed
    ...
    async def test_pattern_fails_all_mode(self):
        ...
        result = await plugin.evaluate({"email": "invalid", "phone": "+1234567890"})
        assert result.matched is True  # Failed
    ...
    async def test_any_pattern_match(self):
        ...
        result = await plugin.evaluate({"email": "test@example.com", "phone": "invalid"})
    ...
    async def test_no_patterns_match_any_mode(self):
        ...
        result = await plugin.evaluate({"email": "invalid"})
        assert result.matched is True  # Failed
...
class TestCombinedValidation:
    ...
    async def test_all_checks_pass(self):
        ...
        result = await plugin.evaluate({"id": "123", "email": "test@example.com", "age": 30})
        assert result.matched is False  # Validation passed
    ...
    async def test_fails_at_required_check(self):
        ...
        result = await plugin.evaluate({"id": "123"})  # Missing email
        assert result.matched is True  # Failed
...
class TestNestedValues:
    ...
    async def test_deep_nesting(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(required_fields=["a.b.c.d.e"], field_types={"a.b.c.d.e": "integer"})
        )
        result = await plugin.evaluate({"a": {"b": {"c": {"d": {"e": 42}}}}})
        assert result.matched is False  # Validation passed

    @pytest.mark.asyncio
    async def test_missing_intermediate_key(self):
        """Test that missing intermediate key is handled gracefully."""
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(required_fields=["a.b.c"]))
        result = await plugin.evaluate({"a": {"x": 1}})  # Missing 'b'
        assert result.matched is True  # Failed
        assert "Missing required fields" in result.message
    ...
    async def test_constraints_on_nested_fields(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_constraints={"user.age": {"min": 0, "max": 120}})
        )
        result = await plugin.evaluate({"user": {"age": 30}})
        assert result.matched is False  # Validation passed

        result = await plugin.evaluate({"user": {"age": 150}})
        assert result.matched is True  # Failed
...
class TestEnumCaseSensitivity:
    ...
    async def test_enum_case_sensitive_default(self):
        ...
        plugin = JSONControlEvaluatorPlugin(
            JSONControlEvaluatorPluginConfig(field_constraints={"status": {"enum": ["active", "inactive"]}})
        )
        # Should fail with "Active" (wrong case)
        result = await plugin.evaluate({"status": "Active"})
    ...
    async def test_enum_case_insensitive_enabled(self):
        ...
        result = await plugin.evaluate({"status": "INACTIVE"})
        assert result.matched is False  # Validation passed

        result = await plugin.evaluate({"status": "pending"})
    ...
    async def test_enum_case_insensitive_non_strings(self):
        ...
        result = await plugin.evaluate({"code": 1})
        assert result.matched is False  # Validation passed

        result = await plugin.evaluate({"code": 4})
        assert result.matched is True  # Failed validation
...
class TestPatternFlags:
    ...
    async def test_pattern_case_sensitive_default(self):
        ...
        plugin = JSONControlEvaluatorPlugin(JSONControlEvaluatorPluginConfig(field_patterns={"code": "^[A-Z]{3}$"}))
        result = await plugin.evaluate({"code": "ABC"})
        assert result.matched is False  # Validation passed
    ...
    async def test_pattern_ignorecase_flag(self):
        ...
        result = await plugin.evaluate({"code": "AB"})
    ...
    async def test_pattern_mixed_string_and_dict(self):
        ...
        # Both should work
        result = await plugin.evaluate({"email": "Test@Example.COM", "code": "1234"})
        assert result.matched is False  # Validation passed
