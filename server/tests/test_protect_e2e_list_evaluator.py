...
def test_list_evaluator_denylist_behavior(client: TestClient):
    ...
    # Given: A registered agent with a DenyList rule blocking "rm" and "shutdown"
    rule_data = {
        "description": "DenyList Rule",
        "enabled": True,
    ...
    agent_uuid, rule_name = create_and_assign_policy(client, rule_data, agent_name="DenyListAgent")
    ...
    # Then: The request should be denied (is_safe=False) and match the rule
...
def test_list_evaluator_allowlist_behavior(client: TestClient):
    ...
    # Given: A registered agent with an AllowList rule allowing ONLY "safe_tool"
    # We use match_on="no_match" to trigger the rule (Deny) if the value is NOT found in the list
    rule_data = {
    ...
    agent_uuid, rule_name = create_and_assign_policy(client, rule_data, agent_name="AllowListAgent")
...
def test_list_evaluator_case_insensitive(client: TestClient):
    ...
    # Given: A rule blocking "BlockMe" with case_sensitive=False
    rule_data = {
    ...
    agent_uuid, rule_name = create_and_assign_policy(client, rule_data, agent_name="CaseAgent")
...
def test_list_evaluator_list_input_any_match(client: TestClient):
    ...
    # Given: A rule blocking request if ANY tag is "restricted"
    rule_data = {
    ...
    agent_uuid, _ = create_and_assign_policy(client, rule_data, agent_name="TagAgent")
...
def test_list_evaluator_list_input_all_match(client: TestClient):
    ...
    # Given: A rule requiring ALL tags to be "safe_tag" or "audit_approved"
    ...
    agent_uuid, _ = create_and_assign_policy(client, rule_data, agent_name="SafeTagAgent")
...
def test_list_evaluator_disallow_tool_name(client: TestClient):
    ...
    # Given: A rule blocking "delete_user" and "drop_db" tools
    rule_data = {
    ...
    agent_uuid, rule_name = create_and_assign_policy(client, rule_data, agent_name="NoDangerousTools")
...
def test_list_evaluator_allow_only_argument_values(client: TestClient):
    ...
    # Given: A rule allowing only "us-east-1" or "us-west-2" for "region" argument
    rule_data = {
    ...
    agent_uuid, rule_name = create_and_assign_policy(client, rule_data, agent_name="RegionPolicy")
...
def test_list_evaluator_edge_cases(client: TestClient):
    """Test edge cases: Empty inputs, Empty rules, Type coercion, Special chars."""
    # 1. Empty Rule Values
    # Given: Rule with empty values list
    rule_empty = {
        "description": "Empty Values Rule",
        "enabled": True,
        "applies_to": "tool_call",
        "check_stage": "pre",
        "selector": {"path": "tool_name"},
    ...
    agent_uuid, _ = create_and_assign_policy(client, rule_empty, agent_name="EmptyRuleAgent")
    ...
    # Given: Rule with integer values
    rule_types = {
        "description": "Type Coercion Rule",
        "enabled": True,
        "applies_to": "tool_call",
        "check_stage": "pre",
    ...
    agent_uuid, rule_name = create_and_assign_policy(client, rule_types, agent_name="TypeAgent")

    # When: Input is integer 10
    req_int = ProtectionRequest(
        agent_uuid=agent_uuid,
        payload=ToolCall(tool_name="count", arguments={"count": 10}, output=None),
        check_stage="pre"
    )
    resp = client.post("/api/v1/protect", json=req_int.model_dump(mode="json"))
    # Then: Should match (deny)
    assert resp.json()["is_safe"] is False

    # When: Input is string "20"
    req_str = ProtectionRequest(
        agent_uuid=agent_uuid,
        payload=ToolCall(tool_name="count", arguments={"count": "20"}, output=None),
        check_stage="pre"
    )
    resp = client.post("/api/v1/protect", json=req_str.model_dump(mode="json"))
    # Then: Should match (deny) - coercion works
    assert resp.json()["is_safe"] is False

    # 3. Special Characters
    # Given: Rule with regex special characters
    rule_special = {
    ...
    agent_uuid, rule_name = create_and_assign_policy(client, rule_special, agent_name="SpecialCharAgent")

    # When: Input exactly matches "(test)"
    req_special = ProtectionRequest(
        agent_uuid=agent_uuid,
        payload=ToolCall(tool_name="search", arguments={"query": "(test)"}, output=None),
        check_stage="pre"
    )
    resp = client.post("/api/v1/protect", json=req_special.model_dump(mode="json"))
    # Then: Should match (deny)
    assert resp.json()["is_safe"] is False

    # When: Input is "test" (without parens)
    req_normal = ProtectionRequest(
        agent_uuid=agent_uuid,
        payload=ToolCall(tool_name="search", arguments={"query": "test"}, output=None),
        check_stage="pre"
    )
    resp = client.post("/api/v1/protect", json=req_normal.model_dump(mode="json"))
    # Then: Should NOT match (safe)
    assert resp.json()["is_safe"] is True

    # 4. Empty/None Input
    # Given: Rule looking for "null" or empty string?
    # Actually testing if rule blows up on None/Empty input
    rule_null = {
        "description": "Null Input Rule",
        "enabled": True,
        "applies_to": "tool_call",
        "check_stage": "pre",
    ...
    agent_uuid, _ = create_and_assign_policy(client, rule_null, agent_name="NullAgent")

    # When: Selector returns None
    req_null = ProtectionRequest(
        agent_uuid=agent_uuid,
        payload=ToolCall(tool_name="check", arguments={}, output=None),
        check_stage="pre"
    )
    resp = client.post("/api/v1/protect", json=req_null.model_dump(mode="json"))
...
def test_list_evaluator_re2_corner_cases(client: TestClient):
    ...
    agent_uuid, _ = create_and_assign_policy(client, rule_large, agent_name="LargeListAgent")
...
def test_list_evaluator_newline_strictness(client: TestClient):
    ...
    # Given: Rule matching "exact"
    rule_strict = {
        "description": "Strict Match Rule",
        "enabled": True,
        "applies_to": "tool_call",
        "check_stage": "pre",
    ...
    agent_uuid, _ = create_and_assign_policy(client, rule_strict, agent_name="StrictAgent")

    # When: Sending "exact\n" (trailing newline)
    req_newline = ProtectionRequest(
        agent_uuid=agent_uuid,
        payload=ToolCall(tool_name="check", arguments={"val": "exact\n"}, output=None),
        check_stage="pre"
    )
    resp = client.post("/api/v1/protect", json=req_newline.model_dump(mode="json"))
